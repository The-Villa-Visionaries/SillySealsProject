...
type StatusType = 'in-progress' | 'resolved' | 'closed'
...
export default function ViewTicketPage({ ticket: propTicketId, onSuccess, onCancel }: ViewTicketPageProps) {
    ...
    const [staff, setStaff] = useState<number | boolean>(false)
    ...
    const handleUpdate = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`http://0.0.0.0:8000/api/ticket-${ticketId}/update`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ticketId, requestId: currentUserId, title, description, category, status })
            })
    ...
    const handleAbandon = async () => {
        ...
        setError(null)
        try {
            const response = await fetch(`http://0.0.0.0:8000/api/ticket-${ticketId}/abandon`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ticketId, requestId: currentUserId})
            })
            if (!response.ok) {
                const errData = await response.json()
                throw new Error(errData.detail || 'Failed to Abandon ticket')
            }
            navigate('/staff')
        } catch (err: any) {
            setError(err.message)
    ...
    return (
        <form onSubmit={handleUpdate} className="overflow-x-hidden min-h-screen bg-[#14452F] flex flex-col items-center relative">
            ...
            <PageWrapper title={staff == currentUserId ? "Assigned Ticket" : "Assign a Ticket"}>
                    ...
                    <div className="flex items-center justify-between w-[85vw] max-w-md gap-4">
                        {status == 'closed' ? (
                            <button
                                ...
                                className="w-full h-[65px] bg-gradient-to-b from-[#14452F] to-[#1A5407] rounded-[18px] text-white font-black italic text-[20px] shadow-md active:scale-95 transition-transform disabled:opacity-50">
                                Close Ticket
                            </button>
                        ) : (
                            <>
                                <button
                                    type="button"
                                    onClick={staff ? () => handleAbandon() : handleBack}
                                    disabled={isSubmitting}
                                ...
                                <button
                                    ...
                                    {staff ? 'Update' : isSubmitting ? "Assigning..." : "Assign Myself"}
                                </button>
                            </>
                        )}
                    </div>
